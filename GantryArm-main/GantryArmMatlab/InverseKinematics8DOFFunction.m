...
clear
% 打开文件以写入Markdown格式
fileID = fopen('D:\Git\GantryArm\GantryArmMatlab\InverseKinematics8DOFFunction1.md', 'w');  % 创建并打开output.md文件
...
syms a1 a2 a3 a4 a5 a6 a7 a0;
syms d3 d4 d5 d6 d7;
a = sym([a0,a1,a2,a3, a4, a5, a6, a7]);   % Link lengths

% Define MDH parameters as symbolic variables
%a = sym([0,0,0,0, 200, 0,0, 0]);   % Link lengths
alpha = sym([pi/2,pi/2,pi/2,0, pi/2, pi/2, pi/2,  pi/2]);  % Twist angles
%d = sym([d0,d1,d2,d3-295.5, d4, d5+152.5+228.5,d6, d7+129+Load]);  % Offsets
d = sym([d0,d1,d2,d3, d4, d5,d6, d7]);  % Offsets
...
%     fprintf(fileID, '$$\n\n');  % End of LaTeX block
% Output T0i (Transformation matrix from base to joint i) as LaTeX in Markdown
fprintf(fileID, '## $^0\\mathbf{T}_%d$\n\n', i);  % Title for each transformation matrix

fprintf(fileID, '$$^{%d}\\mathbf{T}_%d = \\operatorname{Rot}_x(\\alpha_{%d}) \\cdot \\operatorname{Trans}_x(a_{%d}) \\cdot \\operatorname{Rot}_z(\\theta_%d) \\cdot \\operatorname{Trans}_z(d_%d)\\\\\n', i-1, i, i-1, i-1, i, i);

fprintf(fileID, '\\begin{align}\n');
fprintf(fileID, '    =\n');
fprintf(fileID, '    \\begin{bmatrix}\n');

% Extract matrix elements and write them
for row = 1:4
    for col = 1:4
        fprintf(fileID, '        %s', latex(T0(row, col)));
        if col < 4
            fprintf(fileID, ' & ');
        end
    end
...
% Output Ti,i-1 (Transformation matrix from joint i-1 to joint i) as LaTeX in Markdown
fprintf(fileID, '## $^{%d}\\mathbf{T}_{%d}$\n\n', i-1, i);  % Title for Ti,i-1 matrix
% Extract matrix elements and write them
for row = 1:4
    for col = 1:4
        fprintf(fileID, '        %s', latex(T0(row, col)));
