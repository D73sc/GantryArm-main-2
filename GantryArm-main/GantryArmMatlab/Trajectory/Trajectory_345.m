...
%     N = 1;  % 样本数量
    ...
    % 生成随机参数矩阵
    rng('default'); % 保证结果可重复
    param_matrix = limit_min' + (limit_max - limit_min)' .* rand(1, 8);
    % 预分配结果存储
    ...
    % 解析当前参数
    current_params = num2cell(param_matrix(1, :));
    ...
    oz = T_goal(3,2);  
    nz = T_goal(3,1);  
  % 示iacute：生成theta3、theta4、theta5的网格组合（按限位弧度范围遍历）
    step = 100; % 调节步进以提高效率
    offset=0.01;
    theta3_values = linspace(lim3_min+offset, lim3_max-offset,step);
    theta4_values = linspace(lim4_min+offset, lim4_max-offset,step);
    theta5_values = linspace(lim5_min+offset, lim5_max-offset,step);

    [grid_theta3, grid_theta4, grid_theta5] = ndgrid(theta3_values, theta4_values, theta5_values);

    param_matrix = [zeros(size(grid_theta3(:))) ... % d0暂设为0
                    zeros(size(grid_theta3(:))) ... % d1
                    zeros(size(grid_theta3(:))) ... % d2
                    grid_theta3(:) grid_theta4(:) grid_theta5(:) ...
                    zeros(numel(grid_theta3(:)),2)]; % theta6/7暂设为0
    N = size(param_matrix, 1);
% N=1;
%% ================ 批量测试循环 ================
% 在循环前添加以下代码启用多线程
if isempty(gcp('nocreate'))
    parpool('local'); % 在循环前开启并行池
end
    result_cell = cell(N,24);  % 最终存储Excel数据

for n = 1:N
    theta3_val=grid_theta3(n);
...
%% ================ 统计与输出 ================
    % 提取成功索引
    is_success_all = cell2mat(result_cell(:, end)); % is_success在最后一列
    success_idx = find(is_success_all);
    theta3_success = param_matrix(success_idx, 5); 
    theta4_success = param_matrix(success_idx, 6);
    theta5_success = param_matrix(success_idx, 7);
% % 样本成功率
...
%% ================ 可视化 ================
% 转换为度数（可选）
theta3_deg = rad2deg(theta3_success);
theta4_deg = rad2deg(theta4_success);
theta5_deg = rad2deg(theta5_success);

% 绘制三维散点图
figure('Position', [100, 100, 800, 600]);
scatter3(theta3_deg, theta4_deg, theta5_deg, 40, 'filled', 'MarkerEdgeColor', 'k');
xlabel('Theta3 (deg)');
ylabel('Theta4 (deg)');
zlabel('Theta5 (deg)');
title('Theta3-4-5有效解空间');
grid on;
view(45, 30); % 调整视角
...
%     % 解的唯一性检查
%     angle_diff = abs(theta7_1 - theta7_2);
%     if abs(angle_diff - pi) < 1e-6
%         fprintf('\n注意：两个解为镜像解（相差180度）\n');
%     elseif angle_diff < 1e-6
%         error('两解重合，可能遇到奇异位形');
%     else
%         fprintf('\n发现两个不同解（相差%.1f度）\n', rad2deg(angle_diff));
%     end
