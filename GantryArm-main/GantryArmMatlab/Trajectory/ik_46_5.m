...
for n = 1:N
    ...
    %% ================ calculate ================
        ...
        [theta5_sols,theta7_sols] = deal(zeros(2,1));
        try
            [theta5_sols(1), theta5_sols(2)] = theta5_solver(theta4_val, theta6_val, az);
        catch
            continue;
        end
        total_solutions = 0;
        for t5_idx = 1:2
            ...
            try
                [theta7_sols(1), theta7_sols(2)] = theta7_solver(theta4_val, theta5_sols(t5_idx), oz, nz);
            catch
                continue;
            end
            for t7_idx = 1:2
                ...
                try
                    [theta3_1, theta3_2] = theta3_solver2(theta4_val, theta5_ik, theta7_ik, oy, ny);
...
%         fprintf('There are %d sets of solutions\n', success_count);
    ...
    scatter3( theta4_ds, theta6_ds, theta5_ds,40, 'filled', 'MarkerEdgeColor', 'k');
    xlabel('Theta4 (deg)');
    ylabel('Theta6 (deg)');
    zlabel('Theta5 (deg)');
    title('Theta4-6-5');
...
function [theta5_1, theta5_2] = theta5_solver(theta4, theta6, az)
    % 输入参数
    ...
    theta5_1 = wrapToPi(acos(term));
    theta5_2 = wrapToPi(-theta5_1);
