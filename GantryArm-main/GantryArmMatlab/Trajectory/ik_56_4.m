...
for n = 1:N
    ...
    theta6_values = linspace(lim6_min+offset, lim6_max-offset,step);
    [grid_theta5, grid_theta6] = ndgrid(theta5_values, theta6_values);
        param_matrix = [zeros(size(grid_theta5(:))) ... % d0
                    zeros(size(grid_theta5(:))) ... % d1
                    zeros(size(grid_theta5(:))) ... % d2
                    zeros(size(grid_theta5(:))) ... % theta3
                    grid_theta5(:) grid_theta6(:) ...
                    zeros(numel(grid_theta5(:)),2)]; % theta6/7
    ...
    for n = 1:N_ds
        ...
        theta6_val=grid_theta6(n);
        
    %% ================ calculate ================
    
    

        [theta4_sols,theta7_sols] = deal(zeros(2,1));
        try
            [theta4_sols(1), theta4_sols(2)] = theta4_solver(theta5_val, theta6_val, az);
        catch
            continue;
        end
        total_solutions = 0;
        for t4_idx = 1:2
            theta4_ik=theta4_sols(t4_idx);
            try
                [theta7_sols(1), theta7_sols(2)] = theta7_solver(theta4_ik, theta5_val, oz, nz);
            catch
                continue;
            end
            for t7_idx = 1:2
                ...
                try
                    [theta3_1, theta3_2] = theta3_solver2(theta4_ik, theta5_val, theta7_ik, oy, ny);
                    ...
                    for t3_case = 1:2
                        ...
                        joint_solutions(total_solutions).theta4  = theta4_ik;
        ...
        for sol_idx = 1:total_solutions
            ...
            joint_solutions(sol_idx).theta6=theta6_val;
            sol = joint_solutions(sol_idx);
            try
                d0_ik = d0_solver(sol.theta3, sol.theta4, py, ay, Load);
                d1_ik = d1_solver(sol.theta3, sol.theta4, px, ax, Load);
                d2_ik = d2_solver(sol.theta4, pz, az, Load);
    ...
    scatter3( theta5_ds, theta6_ds, theta4_ds,40, 'filled', 'MarkerEdgeColor', 'k');
    xlabel('Theta5 (deg)');
    ylabel('Theta6 (deg)');
    zlabel('Theta4 (deg)');
    title('Theta5-6-4');
...
function [theta4_1, theta4_2] = theta4_solver(theta5, theta6, az)
    % 参数安全处理
    c6 = cos(theta6); 
    s6 = sin(theta6);
    s5 = sin(theta5); 
    ...
    % 多个解的生成
    theta4_1 = (-phi + base_angle);
    theta4_2 = (-phi + pi - base_angle);
