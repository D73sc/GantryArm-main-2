...
for n = 1:N
    ...
    %% ================ calculate ================
        ...
        [theta4_sols,theta6_sols] = deal(zeros(2,1));
        try
            [theta4_sols(1), theta4_sols(2)] = theta4_solver(theta5_val, theta7_val, oz,nz);
        catch
            continue;
        end
        total_solutions = 0;
        for t4_idx = 1:2
            ...
            try
                [theta6_sols(1), theta6_sols(2)] = theta6_solver(theta4_ik, theta5_val, az);
            catch
                continue;
            end
            for t6_idx = 1:2
                ...
                try
                    [theta3_1, theta3_2] = theta3_solver2(theta4_ik, theta5_val, theta7_val, oy, ny);
...
%         fprintf('���ҵ� %d ����Ч�⣺\n', success_count);
    ...
    scatter3( theta5_ds, theta7_ds, theta4_ds,40, 'filled', 'MarkerEdgeColor', 'k');
    xlabel('Theta5 (deg)');
    ylabel('Theta7 (deg)');
    zlabel('Theta4 (deg)');
    title('Theta5-7-4');
...
function [theta4_1, theta4_2] = theta4_solver(theta5, theta7, oz,nz)
    % 输入参数
    ...
    theta4_1 = wrapToPi(acos(term));
    theta4_2 = wrapToPi(-theta4_1);
