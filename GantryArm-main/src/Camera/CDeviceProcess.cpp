...
void CDeviceProcess::SavePicture(CImageDataPointer& objImageDataPointer)
{
	try
	{
		string      strFileName = "";          ///< 图像保存路径名称
		string      strFilePath = "";          ///< 文件路径
		strFilePath = m_strSavePath;

		//创建保存图像的文件夹
        BOOL bRet = CreateDirectory(stringToLPCWSTR(strFilePath), NULL);

        strFileName = strFilePath + "/" + m_strDeviceSN.c_str() + m_strFileName + ".bmp";

		//保存图像为BMP
		m_pBitmap->SaveBmp(objImageDataPointer, &strFileName[0]);
	}
	catch (std::exception)
	{
